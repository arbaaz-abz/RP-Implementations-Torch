...
# Build vocabulary
german_vocab = torchtext.vocab.build_vocab_from_iterator((tokenizer_german(sentence) for sentence, _ in combined_data),
                                                         specials=[PAD_TOKEN, SOS_TOKEN, EOS_TOKEN, UNK_TOKEN],
                                                         min_freq=2, max_tokens=8500)
english_vocab = torchtext.vocab.build_vocab_from_iterator(
    (tokenizer_english(sentence) for _, sentence in combined_data),
    specials=[PAD_TOKEN, SOS_TOKEN, EOS_TOKEN, UNK_TOKEN], min_freq=2, max_tokens=6500)
...
train_data_processed = [
    (preprocess_sentence(example[0], tokenizer_german), preprocess_sentence(example[1], tokenizer_english)) for example
    in train_data]
